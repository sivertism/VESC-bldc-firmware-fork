...
static uint8_t transmit_buffer[4];
			...
			if (rxmsg.IDE == CAN_IDE_EXT) {
				...
				if (id == 255 || id == app_get_configuration()->controller_id) {
					...
					case CAN_PACKET_GET_CURRENT:
						/* Send current split into 4 bytes and scaled by a factor of 1000.
						 * Send with standard 11-bit ID with base 0x500 and offset = 0. */

						ind = 0;
						buffer_append_float32(transmit_buffer, mc_interface_read_reset_avg_motor_current(), 1000.0, &ind);
						comm_can_transmit_std_id(app_get_configuration()->controller_id | 0x500 , transmit_buffer, 4);
						break;

					case CAN_PACKET_GET_RPM:
						/* Send rpm into 4 bytes and scaled by a factor of 1.
...
void comm_can_transmit_std_id(uint32_t id, uint8_t *data, uint8_t len) {
#if CAN_ENABLE
	CANTxFrame txmsg;
